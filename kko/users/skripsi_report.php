...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="assets/css/a4_potrait.css">
...
<th class="tdend">Pembimbing I</th>
  ...
  $year = date('Y');
	...
	if($bulan_ini<=6){
      ...
      $nim=$_SESSION['login'];
$query=mysqli_query($con,"select * from tblkonsultasi WHERE iddosen in ( SELECT iddosen FROM krs where krs.semester='".$_GET['semester']."' and krs.idmatakuliah='SI4426' and krs.idusers='".$_SESSION['login']."' and krs.pembimbing='1') and idmatakuliah='SI4426' and idusers='".$_SESSION['login']."' and semester='".$_GET['semester']."' and sts_konsul='selesai' order by regDate ASC ");
...
<th class="tdend">Pembimbing II</th>
  ...
  $year = date('Y');
	...
	if($bulan_ini<=6){
      ...
      $semester=$string_gabungan;
      $nim=$_SESSION['login'];
$query=mysqli_query($con,"select * from tblkonsultasi WHERE iddosen in ( SELECT iddosen FROM krs where krs.semester='".$_GET['semester']."' and krs.idmatakuliah='SI4426' and krs.idusers='".$_SESSION['login']."' and krs.pembimbing='2') and idmatakuliah='SI4426' and idusers='".$_SESSION['login']."' and semester='".$_GET['semester']."' and sts_konsul='selesai' order by regDate ASC ");
