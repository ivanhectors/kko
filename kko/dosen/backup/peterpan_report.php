...
$semester=intval($_GET['semester']);
$klppeterpan=intval($_GET['klppeterpan']);
$query=mysqli_query($con,"select k.idkelompok,group_concat(k.idusers ORDER BY k.idusers ASC SEPARATOR ',') AS logged_in_users,k.iddosen,k.semester,k.statuskehadiran,k.postingDate from kehadiran k join catatankelompok c on k.postingDate=c.postingDate where k.idkelompok='$klppeterpan' and k.iddosen='".$_SESSION['dlogin']."' and k.semester='$semester' group by c.postingDate");
$cnt=1;
...
?>
  ...
  <tr>
    <td rowspan="2"><center><?php echo htmlentities($cnt);?></td>
    <td rowspan="2"><center><?php 
...
$statusnilai= htmlentities($row['statuskehadiran']);
$nim=htmlentities($row['logged_in_users']);
$mbs_colors = explode(',', $nim);
...
if($statusnilai =="1"){
  ...
  echo  '<label class="badge badge-success" title="Hadir">'.$color.'</label>';
}
else{

  echo '<label class="badge badge-error" title="Tidak Hadir" >'.$color.'</label>';
}
